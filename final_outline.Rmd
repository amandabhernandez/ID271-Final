...
ggpredict(mod2$gam, terms = "TEMPC24H") %>% 
  plot()

ggpredict(mod2$gam, terms = "BC24H") %>% 
  plot()
...
broom.mixed::tidy(mod2$gam, parametric = TRUE, conf.int = TRUE) %>% 
...
mod2_warmcold <- gamm(log(SBP) ~ TEMPC24H + BC24H + RHUM24H + 
                    WARMCOLD*TEMPC24H + 
                    # control for seasonality 
                    SINETIME + COSTIME + WKDAY,
                  random = list(ID =  ~ 1),
...
as_flextable(mod2_warmcold$gam)

warmcold0 <- ggpredict(mod2_warmcold$gam, terms = "TEMPC24H", condition = c(WARMCOLD = "0"))
warmcold1 <- ggpredict(mod2_warmcold$gam, terms = "TEMPC24H", condition = c(WARMCOLD = "1"))

emm_plot <- ggplot(warmcold0) +
